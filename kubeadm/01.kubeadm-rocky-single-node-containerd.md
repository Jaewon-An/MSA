...
### 2-3. Private Registry용 CRI 플러그인 설정 (선택)

02번 문서의 Private Docker Registry(HTTP 또는 self-signed HTTPS)에서 이미지를 pull할 경우, containerd에 해당 레지스트리 주소를 등록해야 합니다. 그렇지 않으면 `x509: certificate signed by unknown authority` 또는 HTTPS 접속 오류로 ImagePullBackOff가 발생할 수 있습니다.

**파일:** `/etc/containerd/config.toml`  
아래 블록을 **파일 맨 아래**에 추가합니다. `<REGISTRY_IP>`는 실제 레지스트리 주소(예: `172.16.0.194`)로 바꿉니다.

```toml
# Private Registry (HTTP 또는 self-signed TLS) — 02번 문서 Registry 사용 시
...
> 적용 전에 `config.toml`에 이미 `[plugins."io.containerd.grpc.v1.cri".registry]` 섹션이 있으면, 그 안에 `configs."<REGISTRY_IP>:5000"` 블록만 병합합니다.
...
- 게이트웨이: 172.16.200.254  
- 기본 DNS: 168.126.63.1  

호스트 대역이 172.16.0.0/16이므로 Pod 네트워크는 겹치지 않는 `10.244.0.0/16`(Flannel 기본)을 사용합니다.
