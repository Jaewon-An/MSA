...
## 3단계 (대안): 04번 Jenkins에서 이미지 빌드하기

> **Jenkins 설정부터 배포까지 전체 흐름**은 **08번 문서 [Jenkins로 Tomcat WAR Kubernetes 배포 가이드](08.jenkins-tomcat-deploy.md)**를 참고하세요. 플러그인·Credentials·Pipeline Job·kubectl 배포 단계까지 포함합니다.

07번에서 사용하는 Tomcat+WAR 이미지를 **04번 문서의 Jenkins** 파이프라인에서 빌드·푸시하는 방법입니다. 소스에서 WAR 빌드 후 동일한 Dockerfile로 이미지를 만들어 Private Registry에 푸시합니다.

### 사전 조건

- 04번 문서 완료: Jenkins가 Kubernetes(또는 별도 서버)에 배포되어 있고, Pipeline·Docker Pipeline 플러그인 설치됨
- 02번 문서 완료: Private Registry 동작 중 (예: `$SERVER_IP:5000`)
- Jenkins에 Registry 인증 정보 등록: Manage Jenkins → Credentials → Username/Password (ID 예: `registry-credentials`)
- 빌드할 애플리케이션 소스가 Git 등 SCM에 있고, Jenkins에서 해당 저장소 접근 가능

### 저장소에 넣을 파일

1. **Dockerfile** (07번 2단계와 동일)  
   프로젝트 루트 또는 `docker/` 등 하위 디렉터리에 두고, **빌드 산출물 WAR를 `app.war`로 복사**하는 형태로 둡니다.
...
2. **Jenkinsfile**  
   아래 예시는 Maven으로 WAR 빌드 → `app.war`로 복사 → Docker 이미지 빌드 → Registry 푸시 순서입니다. Registry 주소·이미지 이름·Credentials ID는 환경에 맞게 수정합니다.

### Jenkinsfile 예시 (Tomcat WAR → 이미지 빌드·푸시)

Pipeline Job에서 **Pipeline script from SCM**으로 이 Jenkinsfile을 사용합니다. Script Path는 저장소 내 경로(예: `Jenkinsfile`)로 지정합니다.

```groovy
pipeline {
  agent any
  environment {
    // 02번 Private Registry 주소 (예: 노드 IP:5000). Jenkins가 접근 가능한 IP/호스트명으로 설정
  ...
  stages {
    ...
    stage('Build WAR') {
      steps {
        ...
        // 산출물 경로는 프로젝트에 맞게 수정 (예: target/*.war)
...
- `REGISTRY`: 02번 문서의 Private Registry 주소 (예: `192.168.1.100:5000`). Jenkins Agent가 도달 가능한 IP/호스트명.
- `APP_NAME`: 07번의 애플리케이션명(예: `h-cms`). 이미지 이름 및 배포 매니페스트의 `image`와 일치시키면 됨.
- `target/*.war`: Maven 기본 경로. Gradle 등이면 `build/libs/*.war` 등으로 변경.
- `registry-credentials`: Manage Jenkins → Credentials에 등록한 Registry Username/Password의 ID.

**참고:** `https://` 없이 사용하는 Registry(일반적으로 사내 5000 포트)인 경우, Jenkins에 "Insecure Registry" 설정이 필요할 수 있습니다. Docker Pipeline이 호스트의 Docker를 쓰는 경우 해당 Docker 데몬의 `insecure-registries`에 `REGISTRY` 주소를 추가합니다.

### Jenkins에서 실행 후 확인

1. Job 한 번 실행 후 Console Output에서 `Build WAR` → `Docker Build & Push` 단계 성공 여부 확인.
2. Registry에 이미지가 올라갔는지 확인:

   ```bash
   SERVER_IP=$(hostname -I | awk '{print $1}')
   curl -s -u admin:admin123 "http://$SERVER_IP:5000/v2/h-cms/tags/list"
   ```
...
3. 07번 5단계 Deployment의 `image`를 `$REGISTRY/h-cms:latest` 또는 `$REGISTRY/h-cms:$BUILD_NUMBER`로 두고 `kubectl apply` 후 Pod가 정상 기동하는지 확인.

이렇게 하면 04번 Jenkins에서 07번과 동일한 Tomcat+WAR 이미지를 빌드·푸시하고, 07번의 Deployment·Service로 배포할 수 있습니다.
