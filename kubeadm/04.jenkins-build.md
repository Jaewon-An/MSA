...
> **파일 위치:** `jenkins-pv.yaml`은 어디에 두어도 됩니다. `kubectl`을 실행하는 쪽에서 경로만 맞으면 됩니다. 예: Jenkins 매니페스트를 한 폴더에 모을 경우 `~/k8s-jenkins/jenkins-pv.yaml`에 저장한 뒤 해당 디렉터리에서 `kubectl apply -f jenkins-pv.yaml` 실행, 또는 `kubectl apply -f ~/k8s-jenkins/jenkins-pv.yaml` 처럼 전체 경로로 지정합니다.
...
**jenkins-deployment.yaml 수정 후 적용 방법**

1. 파일 수정 후 저장
2. 매니페스트 적용 (Deployment 변경 시 롤링 업데이트됨)

```bash
# jenkins-deployment.yaml이 있는 디렉터리에서
kubectl apply -f jenkins-deployment.yaml

# 적용 상태 확인 (새 Pod가 기동될 때까지 대기)
kubectl rollout status deployment/jenkins -n jenkins
```

변경이 Pod에 반영되지 않을 때는 수동으로 재시작할 수 있습니다.
...
```bash
# 클러스터 노드와 내부 IP 확인 (INTERNAL-IP 열 참고)
kubectl get nodes -o wide
```

단일 노드라면 출력된 노드의 `INTERNAL-IP`(예: 172.16.0.10)가 접속할 주소입니다. 해당 노드 서버에서 브라우저를 쓴다면 `http://localhost:30080` 또는 `http://127.0.0.1:30080`으로도 접속할 수 있습니다.

### 2-3. 초기 비밀번호 확인
Jenkins가 완전히 기동한 뒤에만 `initialAdminPassword` 파일이 생성됩니다. Pod가 `Running`이어도 내부 초기화에는 1~2분 걸릴 수 있으므로, **파일이 없다고 나오면 1~2분 기다린 뒤 다시 시도**하거나, 로그에서 기동 완료 메시지를 확인합니다.

```bash
# 기동 완료 여부 확인 (로그에 "Jenkins is fully up and running" 등이 보이면 시도)
...
> **`runSetupWizard=false`인 경우:** 설정 마법사를 끄면 초기 비밀번호 화면이 없을 수 있어 위 파일이 생성되지 않을 수 있습니다. 최초 1회는 `runSetupWizard=true`로 두고 관리자 계정을 만든 뒤, 필요 시 `false`로 변경합니다.
...
비밀번호 없이 접속되는 경우, 아직 보안이 꺼져 있거나 설정 마법사가 건너뛰어진 상태입니다. 운영 환경에서는 반드시 인증을 설정하는 것이 좋습니다.

1. **관리자 계정 생성·설정**
   - **Manage Jenkins** → **Security** → **Configure Global Security**
   - **Security Realm**: "Jenkins' own user database" 선택 후 **Save**
   - **Manage Jenkins** → **Users**에서 기존 사용자(예: admin) 비밀번호 설정 또는 새 사용자 추가

2. **로그인 필수로 변경**
   - **Configure Global Security** → **Authorization**
   - "Logged-in users can do anything" 또는 "Matrix-based security" 등으로 설정해, 익명 접근을 막고 로그인한 사용자만 사용하도록 지정

3. **(선택) 설정 마법사로 처음부터 설정**
   - `jenkins-deployment.yaml`에서 `JAVA_OPTS`의 `runSetupWizard=true`로 두고 재배포하면, 다음 기동 시 초기 잠금 해제 화면과 설정 마법사가 표시됩니다. 여기서 관리자 계정 생성 및 보안 설정을 진행할 수 있습니다.
