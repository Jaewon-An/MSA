...
Kubernetes Pod가 아닌 **Linux 서버에 Jenkins를 직접 설치**하고, 소스 코드 빌드·테스트·이미지 빌드 파이프라인을 구성하는 가이드입니다. OpenJDK 17 기반이며, 01번 문서의 kubeadm 클러스터와 연동할 수 있습니다.
...
2. [사전 요구사항](#사전-요구사항)
3. [1단계: Java 17 설치](#1단계-java-17-설치)
4. [2단계: Jenkins 저장소 추가 및 설치](#2단계-jenkins-저장소-추가-및-설치)
5. [3단계: Jenkins 홈 디렉토리 및 서비스 설정](#3단계-jenkins-홈-디렉토리-및-서비스-설정)
...
## 3단계: Jenkins 홈 디렉토리 및 서비스 설정

기본 `/var/lib/jenkins` 대신 `/data/jenkins`를 사용하도록 변경합니다.
...
# 2. systemd 서비스에서 JENKINS_HOME 변경
